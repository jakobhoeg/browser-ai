...
By default the library appends its own built-in tool-calling scaffold, including the generated tool schemas JSON and the standard `tool_call` / `tool_result` instructions. For advanced use-cases you can insert extra instructions before and/or after that scaffold by passing `toolCallingInstructionsBefore` and/or `toolCallingInstructionsAfter` inside the `providerOptions` call option:
...
When either property is present and non-empty, the shared prompt builder keeps the generated tool schemas JSON and the standard tool-calling instructions, then inserts your custom text before and/or after that scaffold. Empty strings are treated as absent.
