...
|---|---|---|---|
| `toolCallingInstructionsBefore` | `string` | No | Custom prompt text inserted before the built-in tool-calling scaffold. The scaffold still includes the generated tool schemas JSON and the standard `tool_call` / `tool_result` instructions. Empty strings are treated as absent. |

### `toolCallingInstructionsAfter`

| Property | Type | Required | Description |
|---|---|---|---|
| `toolCallingInstructionsAfter` | `string` | No | Custom prompt text inserted after the built-in tool-calling scaffold. The scaffold still includes the generated tool schemas JSON and the standard `tool_call` / `tool_result` instructions. Empty strings are treated as absent. |

If either property is set, the shared prompt builder still emits the generated tool schemas JSON and the standard tool-calling instructions. Your custom text is inserted before and/or after that scaffold.
...
const result = await generateText({
  ...
  providerOptions: {
    "web-llm": {
      ...
      toolCallingInstructionsAfter:
        "Prefer concise answers once the tool results contain enough information.",
